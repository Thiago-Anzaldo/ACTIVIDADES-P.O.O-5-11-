    ...
    internal class AcademiaGastronomia
    {
        ...
        public void PuntuajeIndividualMasAlto()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < calificacionPlatos[i].Length; j++)
                {
                    if (calificacionPlatos[i][j]>=puntajeMasAlto) 
                    {
                        puntajeMasAlto=calificacionPlatos[i][j];
                    }
                }
            }
            ...
            Console.WriteLine("El/los alumno con el puntaje mas alto es/son "+alumnoPuntajeMasAlto+"con "+puntajeMasAlto);
        }
